...
class TrapumDataBase(BaseDBManager):
    ...
    def create_processing(self,pipeline_id,submit_time,process_status,notes):
        """
        ...
        @return  last_id        last inserted primary key value
        """ 
        cols = ["pipeline_id","submit_time","process_status","notes"]
        vals=["%d"%pipeline_id,"'%s'"%submit_time,"'%s'"%process_status,"'%s'"%notes]
        last_id = self.simple_insert("Processings",cols,vals)
        return last_id
